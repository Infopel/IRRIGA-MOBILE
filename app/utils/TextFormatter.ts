export class TextFormatter {
  ...
  static formatToPaperEntityCode(oldText: string, newKey: string): string {
    ...
    return nums.reduce((acc, value, index, array) => {
      ...
      if (index === 6) return acc + "-" + value
      if (index === 12) return acc + "-" + value
      if (index === 14) return acc + "-" + value
      return acc + value
    }, "")
  }
  static formatToDuatCode(oldText: string, newKey: string): string {
    const text = oldText + newKey
    const nums = [...text]
    return nums.reduce((acc, value, index, array) => {
      if (acc.length === 0) return value
