...
describe("Paper Entity Code", () => {
  it("should set valid type 2", () => {
    expect(validate({ type: "paper-entity-code" }, "MZ-INIR-206033-CW-RFB", false)).toBeUndefined()
  })
  it("should set valid type 2 with no hyphens", () => {
    expect(validate({ type: "paper-entity-code" }, "MZINIR206033CWRFB", false)).toBeUndefined()
  })
  it("should set valid type 1", () => {
    expect(validate({ type: "paper-entity-code" }, "MZ-INIR-206033-CS-QCBS", false)).toBeUndefined()
  })
  it("should set valid type 1 with no hyphen", () => {
    expect(validate({ type: "paper-entity-code" }, "MZINIR206033CSQCBS", false)).toBeUndefined()
  })
  it("should set valid type 1 with no hyphen and lowercase", () => {
    expect(validate({ type: "paper-entity-code" }, "mzinir206033csqcbs", false)).toBeUndefined()
  })

  it("should set invalid", () => {
    expect(validate({ type: "paper-entity-code" }, "MZ-INIR-206033-Cs-Q5", false)).toBeTruthy()
    expect(validate({ type: "paper-entity-code" }, "MZ-INIR-206033-Cs-QWE5ss", false)).toBeTruthy()
    expect(validate({ type: "paper-entity-code" }, "MZ-INIR-206033-Css-QWE5s", false)).toBeTruthy()
    expect(validate({ type: "paper-entity-code" }, "MZ-INIR-206033-C-QWE5", false)).toBeTruthy()
  })
})

describe("DUAT", () => {
  it("valid with uppercase", () => {
    expect(validate({ type: "duat" }, "AB/BGUJ", false)).toBeUndefined()
  })
  it("valid with lowercase", () => {
    expect(validate({ type: "duat" }, "ah/edrg", false)).toBeUndefined()
  })
  it("should set valid with no slash bar", () => {
    expect(validate({ type: "duat" }, "aswedf", false)).toBeUndefined()
  })
  it("should set invalid with more than 6 and 1 bar chars", () => {
    expect(validate({ type: "duat" }, "qqq/werf", false)).toBeTruthy()
  })
  it("should set invalid with more than 6 and no bar chars", () => {
    expect(validate({ type: "duat" }, "qwsedrf", false)).toBeTruthy()
  })
  it("should set invalid if has numbers", () => {
    expect(validate({ type: "duat" }, "ab/3erf", false)).toBeTruthy()
  })
})
