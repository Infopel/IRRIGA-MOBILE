...
const RULES: TRule = {
  ...
  duat: yup
    .string()
    .trim()
    .transform((value: string) => (value.length === 0 ? undefined : value.toLocaleUpperCase()))
    .matches(new RegExp("^[A-Z]{2}(|/)[A-Z]{4}$"), "Campo deve ser um número de DUAT"),
  /***
   * PAPER ENTITY CODE
   * MZ-INIR-206033-CS-QCBS
   * MZ-INIR-206033-CW-RFB*/
  "paper-entity-code": yup
    .string()
    .trim()
    .transform((value: string) => (value.length === 0 ? undefined : value.toLocaleUpperCase()))
    .matches(
      /^MZ(-|)INIR(-|)[0-9]{6}(-|)[A-Z]{2}(-|)[A-Z]{3,4}$/,
      "Campo deve ser um número de Código de entidade valido (ex: MZ-INIR-012345-CS-QCBS) ",
    ),
}
