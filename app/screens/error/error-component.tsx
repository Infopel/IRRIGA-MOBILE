...
export const ErrorComponent = (props: ErrorComponentProps) => {
  return (
    <View style={CONTAINER}>
      <Icon as={MaterialIcons} name="bug" />
      <Text style={TITLE_ERROR}>{translate("errorScreen.title")}</Text>
      <Text style={FRIENDLY_SUBTITLE}>{translate("errorScreen.friendlySubtitle")}</Text>
      <View style={ERROR_DETAILS_CONTAINER}>
        <ScrollView>
          <Text selectable style={CONTENT_ERROR}>text={`${props.error}`}</Text>
          {/* <Text selectable style={CONTENT_BACKTRACE} text={`${props.errorInfo.componentStack}`} /> */}
        </ScrollView>
      </View>
      <Button style={BTN_RESET} onPress={props.onReset}>{translate("errorScreen.reset")}</Button>
    </View>
  )
}
